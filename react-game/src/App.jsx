  ...
  const checkAnswer = (e) => {
    ...
    if (!gameOver) {
      ...
      if (correct) {
        ...
        item ? localStorage.setItem(category, JSON.stringify({
          ...item,
        ...
        <>
          {gameOver || userAnswers.length === amount ? (
            <div style={menu} className="menu">
              ...
              <button type="button" className="" onClick={() => setSoundSettings(!soundSettings)}>
                Sound Settings
              </button>
              <button type="button" className="" onClick={() => setStatistic(!statistic)}>
                Statistics
              </button>
            </div>
          ...
          {!gameOver && !loading && userAnswers.length === number + 1 && number !== amount - 1 ? (
            <button type="button" id="next" onClick={nextQuestion}>
              Next Question ( space )
            </button>
          ) : null}
        </>
